...
1. Verify GitHub release assets:
   ...
   ```text
   2fer_X.X.X_darwin_amd64.tar.gz
   2fer_X.X.X_darwin_arm64.tar.gz
   2fer_X.X.X_linux_amd64.tar.gz
   2fer_X.X.X_windows_amd64.zip
   ```
...
1. Review and publish the tap update:
   ...
   cd ../homebrew-tap
   git diff
   git commit -am X.X.X && git push
   ```

1. Test the Homebrew install:
   ```sh
   brew update && brew install gurgeous/tap/2fer
   2fer --version
   ```
